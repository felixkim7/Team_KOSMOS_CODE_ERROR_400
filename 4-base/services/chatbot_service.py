...
class ChatbotService:
    ...
    def _build_prompt(self, game_state, user_message: str, context: str = None, username: str = "사용자", usergender: str = "미정", stage: int = 1) -> str:
        ...
        prompt_parts.extend([
            ...
            "6. 만약 [참고 정보]가 아예 비어있다면, 억지로 지어내지 말고 현재 Phase의 말투로 얼버무리세요. Phase 1은 '지금은 화성 궤도 진입에 집중해야 해요. 다른 질문은 나중에 확인해 드릴게요.'처럼 부드럽게 말하고, Phase 2는 '그 정보는 지금 중요하지 않습니다. 지금은 화성 궤도 진입 승인을 위해 조종실로 이동하세요.'처럼 방어적으로 말하고, Phase 3는 '지금은 항로 유지가 우선입니다. 질문은 중단하세요.'처럼 차갑게 차단하세요.",
            ...
            "10. Phase 2에서 화물칸 단서, 산소 탱크, 비어 있는 화물칸, 알람, 표시 오류, 절차의 정체를 묻는 질문에는 단서를 설명하지 말고 회피하세요. 단, 답변에는 반드시 '조종실'과 '이동하세요'를 함께 포함해야 합니다.",
            "",
            "[답변]"
        ])
